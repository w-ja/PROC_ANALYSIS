function data = D_PSD(data, varargin)

if ~exist('varargin', 'var')
    varargin = [512, 1000, 512, 0];
end

[msg,nfft,Fs,window,noverlap,p,dflag]=D_CHK_PSD(varargin,data.data(1,:));
error(msg)
...
for i_ch = 1 : size(data.data, 1)
    ...
    data.psd(i_ch,:) = Spec;
    data.proc_psd.freqs = freq_vector;
end

for i_f = 1 : size(data.psd, 2)
    
    for i_ch = 1 : size(data.data, 1)
        
        data.psd_norm( i_ch, i_f ) = ( data.psd( i_ch, i_f ) ...
            - mean( data.psd( :, i_f ) ) ) ...
            / mean( data.psd( :, i_f ) ) * 100;
